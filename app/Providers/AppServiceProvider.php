...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        RateLimiter::for('posts', fn (Request $req) =>
            Limit::perMinute(15)->by($req->user()?->id ?: $req->ip())
        );

        // Mensagens privadas: 40 por minuto
        RateLimiter::for('messages', fn (Request $req) =>
            Limit::perMinute(40)->by($req->user()?->id ?: $req->ip())
        );

        // Pedidos de amizade: 20 por minuto
        RateLimiter::for('friends', fn (Request $req) =>
            Limit::perMinute(20)->by($req->user()?->id ?: $req->ip())
        );

        // Likes e comentários: 60 por minuto
        RateLimiter::for('reactions', fn (Request $req) =>
            Limit::perMinute(60)->by($req->user()?->id ?: $req->ip())
        );

        // Criação de bolhas/comunidades: 5 por hora
        RateLimiter::for('create-community', fn (Request $req) =>
            Limit::perHour(5)->by($req->user()?->id ?: $req->ip())
        );
    }
}
