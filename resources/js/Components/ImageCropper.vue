...
onMounted(() => {
    ...
    if (h > maxH) { h = maxH; w = h * props.aspectRatio }
...
// ── drag ──
let dragSX = 0, dragSY = 0, dragSOX = 0, dragSOY = 0

function startDrag(cx, cy) {
    dragging.value = true
    dragSX = cx; dragSY = cy
    dragSOX = ox.value; dragSOY = oy.value
}

function moveDrag(cx, cy) {
    if (!dragging.value) return
    const r = clamp(dragSOX + cx - dragSX, dragSOY + cy - dragSY)
    ox.value = r.x; oy.value = r.y
}
...
<template>
    <!-- Backdrop -->
    <div
        style="position: fixed; inset: 0; z-index: 9999; background: rgba(10,20,35,.88); backdrop-filter: blur(10px); display: flex; align-items: center; justify-content: center; padding: 16px;"
        @click.self="emit('cancel')"
    >
        <!-- Modal card -->
        <div style="background: white; border-radius: 22px; overflow: hidden; box-shadow: 0 32px 96px rgba(0,0,0,.45); display: flex; flex-direction: column; max-width: 100%;">

            <!-- Header -->
            <div style="padding: 18px 24px 12px;">
                <p style="font-size: 14px; font-weight: 800; color: #1a3a4a; margin: 0 0 2px;">Ajusta a imagem</p>
                <p style="font-size: 11px; color: #8ba0b0; margin: 0;">Arrasta para posicionar · scroll ou slider para zoom</p>
            </div>

            <!-- Crop stage (dark bg makes the crop area pop) -->
            <div style="background: #141e2e; padding: 20px 24px; display: flex; justify-content: center; align-items: center;">
                <div
                    :style="{
                        width:        displayW + 'px',
                        height:       displayH + 'px',
                ...
                >
                    ...
                    <div v-else style="position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;">
                        <div class="spin" style="width: 30px; height: 30px; border-radius: 50%; border: 3px solid rgba(0,154,199,.2); border-top-color: #009ac7;" />
                    </div>
                </div>
            </div>
            ...
            <div style="padding: 12px 24px 4px; display: flex; align-items: center; gap: 10px;">
                <!-- minus icon -->
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#b0c0cc" stroke-width="2.5" stroke-linecap="round">
                ...
                <span style="font-size: 11px; color: #009ac7; font-weight: 700; min-width: 36px; text-align: right;">{{ Math.round(scale * 100) }}%</span>
            </div>
            ...
            <div style="padding: 10px 24px 20px; display: flex; gap: 8px; justify-content: flex-end;">
                <button
                    @click="emit('cancel')"
                    style="padding: 9px 20px; border-radius: 10px; border: 1.5px solid #d0e4ee; background: transparent; color: #5a7a8a; font-size: 12px; font-weight: 700; cursor: pointer; transition: background .15s;"
                    @mouseenter="$event.target.style.background='#f0f8ff'"
                    @mouseleave="$event.target.style.background='transparent'"
                >Cancelar</button>
                <button
                    @click="confirm"
                    style="padding: 9px 22px; border-radius: 10px; border: none; background: #009ac7; color: white; font-size: 12px; font-weight: 700; cursor: pointer; box-shadow: 0 3px 12px #009ac730; transition: opacity .2s;"
