...
export function useDrag(onBubbleClick) {
  ...
  function startTouch(bubble, e) {
    ...
    if (bubble.selected) return
    const t = e.touches[0]
    dragging.value  = { id: bubble.id, ox: t.clientX - bubble.x, oy: t.clientY - bubble.y, sx: t.clientX, sy: t.clientY }
    dragMoved.value = false
  }
  ...
  function onMouseMove(e, bubbles) { _move(e.clientX, e.clientY, bubbles) }
