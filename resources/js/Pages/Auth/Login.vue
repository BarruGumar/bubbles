...
<template>
    <GuestLayout>
        ...
        <div
            v-if="status"
            style="background:#edfaf4;border:1px solid #2ea87e44;border-radius:12px;padding:10px 16px;font-size:12px;color:#2ea87e;margin-bottom:20px"
        >
            {{ status }}
        </div>
        ...
        <form @submit.prevent="submit" style="display: flex; flex-direction: column; gap: 16px">
            <div style="display: flex; flex-direction: column; gap: 6px">
                ...
                <input v-model="form.email" type="email" required autofocus autocomplete="username" class="auth-input" />
            ...
            <div style="display: flex; flex-direction: column; gap: 6px">
                <div style="display: flex; justify-content: space-between; align-items: center">
                    ...
                    <Link v-if="canResetPassword" :href="route('password.request')" style="font-size:11px;color:#009ac7;text-decoration:none;font-weight:600">Esqueceste?</Link>
                </div>
                <input v-model="form.password" type="password" required autocomplete="current-password" class="auth-input" />
                ...
                <div style="background:white;border-radius:20px;max-width:400px;width:100%;box-shadow:0 24px 80px rgba(0,0,0,0.25);overflow:hidden">
                    ...
                    <div style="padding:24px">
                        ...
                        <div style="background:#fef2f2;border:1px solid #fca5a5;border-radius:12px;padding:14px 16px;margin-bottom:20px">
                            <p style="font-size:14px;color:#374151;margin:0;line-height:1.55">{{ punishmentModal.reason || 'Sem motivo especificado.' }}</p>
                        </div>
                        <button @click="banModalVisible = false" style="width:100%;padding:13px;border-radius:99px;background:#991b1b;border:none;color:white;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px #991b1b55">
                            Fechar
                        </button>
                    </div>
                </div>
