...
<template>
    ...
    <section>
        ...
        <div style="margin-bottom: 28px;">
            <p style="font-size: 11px; font-weight: 700; color: #5a7a8a; text-transform: uppercase; letter-spacing: .06em; margin: 0 0 8px;">Banner do perfil</p>
            ...
            >
                <span v-if="!bannerPreview" style="font-size: 12px; color: #8ba0b0; background: rgba(255,255,255,.75); padding: 5px 16px; border-radius: 99px; backdrop-filter: blur(4px);">
                    Clica para adicionar banner
                </span>
                <span v-else style="font-size: 12px; color: white; background: rgba(0,0,0,.38); padding: 5px 16px; border-radius: 99px;">
                    Alterar banner
                </span>
            </div>
            ...
            <div v-if="bannerForm.banner" style="display: flex; align-items: center; gap: 10px; margin-top: 10px;">
                <button
                    ...
                    :style="{
                        ...
                        opacity: bannerForm.processing ? .6 : 1, transition: 'opacity .2s',
                    }"
                >{{ bannerForm.processing ? 'A enviar...' : 'Guardar banner' }}</button>
                <Transition name="fade">
                    <span v-if="status === 'banner-updated'" style="font-size: 11px; color: #2ea87e; font-weight: 600;">✓ Guardado</span>
                </Transition>
            </div>
        </div>
        ...
        <div style="display: flex; align-items: flex-start; gap: 20px; margin-bottom: 28px;">
            ...
            <div style="position: relative; flex-shrink: 0; cursor: pointer;" @click="avatarInput.click()" title="Alterar foto">
                <img
                    v-if="avatarPreview"
                    :src="avatarPreview"
                    :style="{
                ...
                <div style="position:absolute;inset:0;border-radius:50%;background:rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity .2s;"
                ...
                <div v-if="avatarForm.avatar" style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                    <button
                        ...
                        :style="{
                            ...
                            background: '#009ac7', color: 'white',
                            fontSize: '12px', fontWeight: '700', cursor: 'pointer',
                            opacity: avatarForm.processing ? .6 : 1,
                        }"
                    >{{ avatarForm.processing ? 'A enviar...' : 'Guardar foto' }}</button>
                    <Transition name="fade">
                        <span v-if="status === 'avatar-updated'" style="font-size:11px;color:#2ea87e;font-weight:600;">✓ Guardado</span>
                    </Transition>
                </div>
        ...
        <form @submit.prevent="form.patch(route('profile.update'))" style="display:flex;flex-direction:column;gap:18px;">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                ...
                <input v-model="form.name" type="text" required autocomplete="name"
                    style="background:#f0f8ff;border:1.5px solid #4ebcff44;border-radius:11px;padding:10px 14px;font-size:13px;color:#1a3a4a;outline:none;font-family:inherit;width:100%;box-sizing:border-box;transition:border-color .2s;"
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                ...
                <div style="position:relative;">
                    ...
                    <input v-model="form.username" type="text" autocomplete="off" placeholder="o_teu_username"
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                ...
                <textarea v-model="form.bio" maxlength="300" rows="3" placeholder="Conta um pouco sobre ti..."
            ...
            <div v-if="mustVerifyEmail && user.email_verified_at === null"
                style="background:#fff9ec;border:1px solid #e0a04044;border-radius:10px;padding:12px 14px;font-size:12px;color:#8a6020;">
                O teu email não está verificado.
                <Link :href="route('verification.send')" method="post" as="button"
                    style="color:#009ac7;font-weight:700;background:none;border:none;cursor:pointer;font-size:12px;">
                    Reenviar verificação.
                </Link>
                <div v-if="status === 'verification-link-sent'" style="margin-top:6px;color:#2ea87e;font-weight:600;">Link enviado!</div>
            </div>
            ...
            <div style="display:flex;align-items:center;gap:14px;">
                ...
                >Guardar</button>
                <Transition name="fade">
                    <span v-if="status === 'profile-updated'" style="font-size:12px;color:#2ea87e;font-weight:600;">✓ Guardado</span>
                </Transition>
            </div>
        </form>
    </section>
